...
public class SignUpActivity extends BaseActivity implements APIConstants{
    ...
    protected void onCreate(Bundle savedInstanceState) {
       ...
       // Creating an Instance ID: String
        iid = InstanceID.getInstance(this).getId();
        Log.i("iid",iid);
        //Creating a GUID: String
    ...
    private void registerRequest(){
       ...
       // mKeys.put(REGISTER_URL_PARAM,edttxtMobile.getText().toString().trim());
        mKeys.put(REGISTER_URL_PARAM_UNIQ1,edttxtMobile.getText().toString().trim());
        mKeys.put(REGISTER_URL_PARAM_UNIQ2,uniqueID);
        ...
        JsonObjectRequest jsonObjReq = new JsonObjectRequest(Request.Method.POST,url,
                new Response.Listener<JSONObject>() {
                    ...
                    public void onResponse(JSONObject response) {
                        ...
                        if(registerGSON.getStatus().equalsIgnoreCase(REQUEST_STATUS_SUCCESS)) {
                            ...
                            saveUserData(registerGSON.getUser_id());
                            openChatScreen(response.toString());
                        }else if(registerGSON.getFailure_message().equalsIgnoreCase(REQUEST_STATUS_FAILURE)){
                            showShortToast("User already exist");
                        }
                        pDialog.hide();
                    }
...
// Adding request to request queue
    ...
    private void saveUserData(String user_id) {
        appPreferences.putString(PREF_USERID,user_id);
